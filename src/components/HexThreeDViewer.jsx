...
import { interpolateHexEdge3D, getHexFlatTriangleVertices, getDirectionAtPoint } from '../utils/hexGeometry3d.js';
import './ThreeDViewer.css';

// Scale factor for the base triangle (80% of original size)
const TRIANGLE_SCALE = 0.8;

// Number of samples per edge for interpolation
const EDGE_SAMPLES_3D = 15;

/**
 * HexThreeDViewer component - renders the hex orbifold path on a puffed 3D surface.
 * @param {Object[]} edges - Array of float edge objects with from/to {side, t}
 * @param {function} onClose - Callback to close the 3D viewer
 */
function HexThreeDViewer({ edges, onClose }) {
  ...
  useEffect(() => {
    ...
    // Create flat triangle base (scaled to 80%)
    const triangleVertices = getHexFlatTriangleVertices();
    const triangleGeometry = new THREE.BufferGeometry();
    const trianglePositions = new Float32Array([
      triangleVertices[0].x * TRIANGLE_SCALE, triangleVertices[0].y * TRIANGLE_SCALE, 0,
      triangleVertices[1].x * TRIANGLE_SCALE, triangleVertices[1].y * TRIANGLE_SCALE, 0,
      triangleVertices[2].x * TRIANGLE_SCALE, triangleVertices[2].y * TRIANGLE_SCALE, 0
    ]);
    ...
    if (edges && edges.length > 0) {
      ...
      for (let edgeIndex = 0; edgeIndex < edges.length; edgeIndex++) {
        ...
        const interpolatedPoints = interpolateHexEdge3D(edge, EDGE_SAMPLES_3D);
    ...
    <div className="three-d-viewer-overlay">
      <div className="three-d-viewer-container">
        <div className="three-d-viewer-header">
          <h2>3D Path View (Hex Orbifold)</h2>
...
export default HexThreeDViewer;
