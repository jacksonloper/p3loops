...
import { HexLoopIterator } from '../utils/hexLoopIterator.js';
...
/**
 * HexLoopSelector - A dropdown component for selecting and loading hex orbifold loops.
 * 
 * Works like the P2 LoopSelector - user picks a loop and it loads directly.
 ...
 */
function HexLoopSelector({ onSelectLoop, disabled = false }) {
  // Create iterator once
  const [iterator] = useState(() => new HexLoopIterator());
  ...
  return (
    <div className="loop-selector">
      <label htmlFor="hex-loop-select">Load Loop:</label>
      <select
        id="hex-loop-select"
        value={selectedValue}
...
export default HexLoopSelector;
