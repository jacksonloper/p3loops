/**
 * Loop Iterator for enumerating all valid loops on the hexagonal orbifold.
 * 
 * This module provides a system for iterating through all possible closed loops
 * on the hexagonal fundamental domain, starting with "AX to BY" to break symmetry.
 ...
 * detection logic as the existing hex combinatorial path logic.
 * 
...
/**
 * The fixed first edge: AX to BY.
 * This breaks the 3-fold symmetry of the hexagonal orbifold.
 * The path always starts at X (≡ Y ≡ Z), adjacent to sides AX and BX.
 */
const FIRST_EDGE_FROM = { startPos: null, endPos: null, side: 'AX' };
const FIRST_EDGE_TO = { startPos: null, endPos: null, side: 'BY' };

/**
 * Create the initial path with the first edge (AX to BY).
...
export class HexLoopIterator {
  ...
  /**
   * Initialize the iterator with the first edge (AX to BY).
   */
  _initialize() {
   ...
   * @returns {boolean} True if new paths/loops were found
   */
  _exploreNextLength() {
 ...
 */
export function hexLoopToFloatEdges(loopEntry) {
 ...
 */
export function describeHexLoop(loopEntry, index) {
