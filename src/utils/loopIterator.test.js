...
import {
  ...
  loopToFloatEdges,
  describeLoop
} from './loopIterator.js';

describe('LoopIterator', () => {
  let iterator;
...
describe('loopToFloatEdges', () => {
  it('should convert a loop to float edges', () => {
    ...
    if (loops.length > 0) {
      const floatEdges = loopToFloatEdges(loops[0]);
...
describe('describeLoop', () => {
  it('should generate a description for a loop', () => {
    ...
    if (loops.length > 0) {
      const description = describeLoop(loops[0], 0);
...
describe('Loop validity', () => {
  it('all discovered loops should start with north to west', () => {
    const iterator = new LoopIterator();
    iterator.exploreToLength(15);
    const loops = iterator.getCurrentLoops();
    ...
    for (const loop of loops) {
      ...
      // First edge should be from north to west
      expect(firstEdge.from.side).toBe('north');
      expect(firstEdge.to.side).toBe('west');
    }
  });
